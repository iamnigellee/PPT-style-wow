...
- **Emphasis:** When choosing how to make an element stand out, reduce weight rather than increase it. A weight-300 headline at 56px commands more attention than a bold one because it signals "I don't need to shout." For any new heading, label, or call-out, default to weight 300 and let size or whitespace do the work. Only use weight 400 for interactive text (buttons, links) where the extra density signals clickability.

- **Depth:** Every elevated element must cast a blue-tinted, multi-layer shadow. The formula is always two layers: a branded blue-gray (`rgba(50,50,93,...)`) at a larger offset and a neutral black (`rgba(0,0,0,...)`) at a smaller offset, both with negative spread. If you're adding a new floating component (tooltip, dropdown, sheet), derive from the Level 3 formula and adjust Y-offset. Never use a single-layer neutral shadow — that belongs to a different design language entirely.

- **Color allocation:** Purple (`#533afd`) means interactive. If the element is not clickable, tappable, or selectable, it does not get purple. When you need a new accent for a non-interactive purpose (decorative illustration, status indicator, background tint), derive it from the navy-to-slate neutral range or the ruby/magenta gradient range — never from the purple. This principle is what gives Stripe's purple its power: scarcity.

- **Spacing and density:** Default to generous. 64px+ between sections, 24px+ inside cards, 16px+ between list items. This brand uses whitespace as a luxury signal — the financial data inside components may be dense, but the chrome around it must breathe. If forced to choose between "slightly too spacious" and "slightly too tight," always choose spacious.

- **Edge treatment:** All corners use conservative radius (4–8px). Nothing pill-shaped, nothing sharp (0px). This applies to every new component: modal corners get 6–8px, tooltip corners get 4px, avatar corners get 50% (circle only). The conservatism is deliberate — it reads as financial-grade precision, not playfulness.

- **Typography contrast:** Create hierarchy through size and tracking, not weight. At 56px, tracking is -1.4px (tight, dense blocks); at 16px, tracking is normal (open, readable). This progressive tracking is the typographic signature — the bigger the text, the tighter the letters. When adding a new text style, calculate tracking as approximately -2.5% of font size for display text, tapering to 0 at body sizes.

- **Section rhythm:** Alternate white (`#ffffff`) and dark navy (`#1c1e54`) backgrounds. The light-dark cadence is the layout's visual heartbeat — it prevents monotony and creates natural content grouping. Any new full-width section should follow the alternation. Never use a third background color (gray, off-white) that would break the binary rhythm.

- **Interactive state philosophy:** Hover states darken, never lighten. Purple buttons hover to a deeper purple (`#4434d4`), not a lighter one. Ghost buttons gain a subtle purple tint (`rgba(83,58,253,0.05)`), not a solid fill. This creates a "pressing into depth" interaction feel rather than a "lighting up" feel.
...
9. **Emphasis through lightness, not weight** -- when a new element needs to command attention, make it larger and lighter (weight 300), not smaller and bolder. This is the brand's core design decision.
10. **Purple = interactive, always** -- if building a new component, only apply `#533afd` to elements the user can click or tap. Non-interactive accents use the navy-slate range or the ruby/magenta decorative range.
11. **Generous spacing by default** -- when unsure about padding or margin, err on the side of more space. The brand reads as premium because its whitespace is luxurious, not economical.
